...
def plot_confusion_matrix_from_long_format(df, category_name, category_labels_data, save_path=None):
    ...
    cbar.set_label('Prediction Accuracy (%)', fontsize=16)  # 设置colorbar标签字体大小
...
# --- 步骤 3: 自动识别类别并生成图表 ---
target_categories = ['lighting type']
single_size = 32  # 更大正方形子图
font_kwargs = {'fontsize': 60}  # 统一大字体
n_files = len(main_dfs)
n_cats = len(target_categories)
fig, axes = plt.subplots(1, n_files, figsize=(single_size * n_files + 8, single_size))
if n_files == 1:
...
for file_idx, (main_df, file_path) in enumerate(zip(main_dfs, file_paths)):
    ...
    if category not in all_categories or category not in category_labels_data:
        ax.axis('off')
        continue
    ...
    sns.heatmap(
        ...
        yticklabels=filtered_class_labels,
        cbar=True,
        annot_kws={'size': 54},  # heatmap内部字体保持不变
        ax=ax
    )
    # 设置标题，左边origin，右边refined
    if file_idx == 0:
        title_prefix = 'Original'
    else:
        title_prefix = 'Refined'
    ax.set_title(f'{title_prefix} {category.title()}', pad=60, **font_kwargs)
    ax.set_ylabel('True Label', **font_kwargs)
    ax.set_xlabel('Predicted Label', **font_kwargs)
    ax.set_xticklabels(ax.get_xticklabels(), rotation=45, ha='right', fontsize=60)
    ax.set_yticklabels(ax.get_yticklabels(), rotation=0, fontsize=60)
    cbar = ax.collections[0].colorbar
    cbar.ax.tick_params(labelsize=60)
    cbar.set_label('Prediction Accuracy (%)', fontsize=60)
plt.subplots_adjust(wspace=1.2)  # 增大子图间距
plt.tight_layout(rect=[0, 0, 0.91, 1])
# 保存为一个pdf文件
script_dir = os.path.dirname(os.path.abspath(__file__))
prefix = os.path.normpath(file_paths[0]).split(os.sep)[-2] if os.sep in os.path.normpath(file_paths[0]) else ''
save_pdf_path = os.path.join(script_dir, f'{prefix}_lighting_type_2files_heatmaps.pdf')
fig.savefig(save_pdf_path, dpi=300, bbox_inches='tight', format='pdf')
print(f"lighting type类别的两个热力图已保存到: {save_pdf_path}")
plt.show()